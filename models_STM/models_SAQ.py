...
class STM(nn.Module):
    ...
    def forward(self, frame, mask=None, keys=None, values=None, num_objects=None, criterion=None, max_obj=None):

        if self.phase == 'test':
            if mask is not None: # keys
                return self.memorize(frame, mask, num_objects)
            else:
                logits, ps, r4 = self.segment(frame, keys, values, num_objects, max_obj)

                out = torch.softmax(logits, dim=1)
                uncertain = torch.max(out,dim=1)[0].unsqueeze(dim=0)
                uncertain = F.interpolate(uncertain.float(), size=[32, 32], mode="bilinear")
                out = torch.argmax(out,dim=1,keepdim=True).float() 
                out = F.interpolate(out.float(), size=[32, 32], mode="bilinear")
                input_qua = torch.cat((r4, out, uncertain),dim=1)
                ...
                return logits, ps, output_quality
            ...
            out = F.interpolate(out.float(), size=[32, 32], mode="bilinear")
            input_qua = torch.cat((r4s, out, uncertainty),dim=1)
            r4 = self.conv1(input_qua)
            r5 = self.res5(r4)
            r5 = self.avgpool(r5)
            r5 = r5.view(-1,2048)
