...
class NegativePoints(object):
    ...
    elem: which element of the sample to choose as the binary mask
    """
    def __init__(self, sigma=10, elem='refine',pad_pixel =10):
        self.sigma = sigma
        self.elem = elem
        self.pad_pixel =pad_pixel
    ...
    def __call__(self, sample):
        ...
        if sample[self.elem].ndim == 3:
            raise ValueError('NegativePoints not implemented for multiple object per image.')
        _target = sample[self.elem]

        targetshape=_target.shape
        if np.max(_target) == 0:
            sample['Negative_points'] = np.zeros([targetshape[0],targetshape[1],1], dtype=_target.dtype) #  TODO: handle one_mask_per_point case
        else:
            _points = helpers.iog_points(_target, self.pad_pixel)
            sample['Negative_points'] = helpers.make_gt2(_target, _points, sigma=self.sigma, one_mask_per_point=False)
        return sample

    def __str__(self):
        return 'NegativePoints:(sigma='+str(self.sigma)+', pad_pixel='+str(self.pad_pixel)+', elem='+str(self.elem)+')'
    ...
    def __call__(self, sample):
        ...
        for elem in self.crop_elems:
            ...
            if self.mask_elem == elem:
                ...
                for k in range(0, _target.shape[-1]):
                    if np.max(_target[..., k]) == 0:
                        _crop.append(helpers.crop_from_mask(_img, sample['gt'], relax=self.relax, zero_pad=self.zero_pad))
                    else:
                        _tmp_target = _target[..., k]
                        _crop.append(helpers.crop_from_mask(_img, _tmp_target, relax=self.relax, zero_pad=self.zero_pad))
            if len(_crop) == 1:
