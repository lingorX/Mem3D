...
class CVCSegmentation(data.Dataset):
    ...
    def __getitem__(self, index):
        _img, _target, _scribble = self._make_img_gt_point_pair(index)
    ...
    def _make_img_gt_point_pair(self, index):
        ...
        _scribble = np.array(Image.open(os.path.join(_scribble_dir, self.image_ids[index].split('-')[0], self.image_ids[index].split('-')[1]))).astype(np.float32)
        _tmp[_tmp!=1] = 0 
        _tmp[_tmp==1] = 255.0
