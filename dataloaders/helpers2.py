...
def iog_points(mask, pad_pixel=10):
    ...
    new_distance = np.ones((h,w),np.uint8)
    new_distance[center_point] = 0
    distance_of_new=ndimage.distance_transform_edt(new_distance)
    distance_of_new[erosion!=255]=0
    raw2,column2=getPositon(distance_of_new)
    second_point = [column2,raw2]

    a=[center_point,second_point]  
...
def make_gt(img, labels, sigma=10, one_mask_per_point=False):
    ...
    if labels is None:
        ...
        if labels.ndim == 1:
            ...
            gt_0 = np.zeros(shape=(h, w), dtype=np.float64)
            gt_1 = np.zeros(shape=(h, w), dtype=np.float64)
            gt_1 = np.maximum(gt_1, make_gaussian((h, w), center=labels[1, :], sigma=sigma))
            gt_0 = np.maximum(gt_0, make_gaussian((h, w), center=labels[0, :], sigma=sigma))
