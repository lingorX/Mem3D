    ...
    for ii, sample_batched in enumerate(testloader):  
        if isinstance(sample_batched,list):
            ...
            for idx,con in enumerate(sample_batched):
                inputs, gts, metas =torch.cat((con['concat'], torch.zeros_like(con['concat'][:,3:4])),1), con['gt'], con['meta']
                inputs = inputs.cuda()
            ...
            # inputs, gts, metas = torch.cat((sample_batched['concat'],torch.zeros_like(sample_batched['concat'][:,3:4])),1), sample_batched['gt'], sample_batched['meta']
